% ----------------------------------------------------------------------- %
% Test case: Anderson's purely subsonic nozzle
% ----------------------------------------------------------------------- %
...
% Numerical data:
N = 150; % Number of grid nodes
sigma = 0.5; % Stability criteria (CFL)
t_end = 50E-2; % [s] - Desired final time
w = 1; % [] - Constant parameter for MUSCL-Hancock's slope vectors (can go from -1 to 1)
...
p_out = 0.93*p_0; % [Pa] - Exit pressure (purely subsonic flow)
...
A_f = @(x) (x <= L/2)*(At*(1 + 2.2*(3*x/L - 1.5)^2)) + ...
    (x > L/2)*(At*(1 + 0.2223*(3*x/L - 1.5)^2)); % [m2] - Cross-section area analytical function
dAdx_f = @(x) (x <= L/2)*(At*2.2*2*3/L*(3*x/L - 1.5)) + ...
    (x > L/2)*(At*0.2223*2*3/L*(3*x/L - 1.5)); % Cross-section area derivative analytical function
...
for i = 1:N
    rho(1,i) = rho_0*(1.0 - 0.023*3*x(i)/L); % [kg/m3] - Density
    T(1,i) = T_0*(1.0 - 0.009333*3*x(i)/L); % [K] - Temperature
    u(1,i) = a_0*(0.05 + 0.11*3*x(i)/L); % [m/s] - Velocity
end
...
% Call the analytical solution for perfect gases considering purely
% subsonic flow:
[M_exact,p_exact,rho_exact,T_exact,u_exact] = Nozzle_subsonic_solution(A_f,At,x_exact,Rg_0,gamma_0,p_0,rho_0,T_0,p_out);
...
Anderson = readtable('RESULTS_Subsonic_nozzle_Anderson.txt','Delimiter','\t');
